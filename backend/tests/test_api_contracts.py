...
from app.domain.errors import DomainError, InvalidOperation, NotFound
...
def test_transaction_handlers_map_invalid_ranges_and_missing_records(finance_service):
    with pytest.raises(DomainError) as invalid_range:
    ...
    assert isinstance(invalid_range.value, InvalidOperation)
    ...
    assert isinstance(missing_update.value, NotFound)

    with pytest.raises(DomainError) as missing_delete:
        transactions_api.delete_transaction(99999, finance_service)
    assert isinstance(missing_delete.value, NotFound)


def test_account_and_transfer_handlers_map_domain_errors(finance_service):
    main_account_id = finance_service.accounts()[0]["id"]

    with pytest.raises(DomainError) as main_account:
        accounts_api.deactivate_account(main_account_id, finance_service)
    assert isinstance(main_account.value, InvalidOperation)

    with pytest.raises(DomainError) as missing_account:
        accounts_api.deactivate_account(99999, finance_service)
    assert isinstance(missing_account.value, NotFound)
    ...
    with pytest.raises(DomainError) as invalid_transfer:
        transfers_api.create_transfer(transfer, finance_service)
    assert isinstance(invalid_transfer.value, InvalidOperation)

    with pytest.raises(DomainError) as missing_transfer:
        transfers_api.delete_transfer(99999, finance_service)
    assert isinstance(missing_transfer.value, NotFound)


def test_budget_and_recurring_handlers_map_missing_records(finance_service):
    with pytest.raises(DomainError) as missing_budget:
        budgets_api.update_budget(
            99999,
            BudgetCreate(category="Food", monthly_limit=100),
            finance_service,
        )
    assert isinstance(missing_budget.value, NotFound)

    with pytest.raises(DomainError) as missing_recurring:
    ...
    assert isinstance(missing_recurring.value, NotFound)
